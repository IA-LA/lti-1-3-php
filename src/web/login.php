...
LTI\LTI_OIDC_Login::new(new Lti_Database(["iss" => $_REQUEST['iss'], "login_hint" => $_REQUEST['login_hint'], "target_link_uri" => $_REQUEST['target_link_uri'], "lti_message_hint" => $_REQUEST['lti_message_hint']]))
    ...
    ->do_oidc_login_redirect(sizeof(preg_split("/\/publicacion\/[a-f,0-9]{24}/", $_REQUEST['target_link_uri'], 0, PREG_SPLIT_NO_EMPTY) ) > 1 ? ($_REQUEST['target_link_uri']) : ("https://ailanto-dev.intecca.uned.es/lti13" . "/launch.php"))
