...
function get_public_key($key_set_url) {
    ...
    foreach ($public_key_set['keys'] as $key) {
        if ($key['kid'] == $_POST['client_assertion']['header']['kid'] && $key['alg'] == $_POST['client_assertion']['header']['alg']) {
            try {
...
$jwt = JWT::decode($_POST['client_assertion'], get_public_key(TOOL_HOST . "/jwks.php"), array('RS256'));
